...
float Gyro_Out = 0;
float Turn_Out= 0;
uint8 MSEventCount = 0;
uint8 SpeedControlCount = 0;
...
{       
         ...
         if(MSEventCount == 1) {
           ...
           SpeedControlCount++;
           if(SpeedControlCount >= SPEED_CONTROL_COUNT) {
             SpeedData();
         ...
         } else if(MSEventCount == 4) {
           
           if(DirectionControlCount >= DIRECTION_CONTROL_COUNT){
