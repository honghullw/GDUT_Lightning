...
uint8 AngleControlPeriod = 0;
float AngleControlOutNew = 0;
float AngleControlOutOld = 0;
float AngleControlOut = 0;

uint8 GyroControlPeriod = 0;
float GyroControlOutNew = 0;
float GyroControlOutOld = 0;
float GyroControlOut = 0;
...
//      if(Realize>0) Realize=0;                //可执行前进
//      if(Realize<-8) Realize=-8;
      
            if(Realize>8) Realize=8;
      if(Realize<-8) Realize=-8;
//            if(Realize>8) Realize=8;
//      if(Realize<0) Realize=0;
...
float Angle_PID(float Angle_Data,float Angle_m,float balance_angle)
{
      ...
      AngleControlOutOld = AngleControlOutNew;
      AngleControlOutNew = Realize;
...
float Gyro_PID(float Gyro_Data,float Gyro_m)
{
      ...
      //积分饱和限幅
      if(Gyro_Error_Count>50000) Gyro_Error_Count=50000;
      else if(Gyro_Error_Count<-50000) Gyro_Error_Count=-50000;
      
      //位置PID计算
      Realize = Gyro.P * Gyro_Error +  Gyro.I * Gyro_Error_Count;
      
      GyroControlOutOld = GyroControlOutNew;
      GyroControlOutNew = Realize;
...
void AngleControlOutput(void) //速度平滑输出函数
{
      float Value;
      Value = AngleControlOutNew - AngleControlOutOld;
      AngleControlOut= Value * (AngleControlPeriod + 1)/10 + AngleControlOutOld;
}

void GyroControlOutput(void) //速度平滑输出函数
{
      float Value;
      Value = GyroControlOutNew - GyroControlOutOld;
      GyroControlOut= Value * (GyroControlPeriod + 1)/5 + GyroControlOutOld;
}
