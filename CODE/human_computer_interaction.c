...
void OledViewPhysicalCondition()//一级界面
{
  ...
  oled_printf_float(50,1，car_physical_condition.angle);//输出小车俯仰角
  
  oled_p6x8str(0,2,"Car Fix Angle");
  oled_printf_float(50,2，angle_data.car_physical_condition.fix_angle);//输出小车修正俯仰角
  
  oled_p6x8str(0,3,"Car Real Position");
  oled_printf_float(50,3，car_physical_condition.position);//小车距离赛道中心距离
                            
                                               
  oled_p6x8str(0,4,"Car Speed");        //小车速度
  oled_printf_float(50,4，car_physical_condition.speed);
...
void OledViewParameters()//二级界面
{
  ...
  oled_p6x8str(0,4,"kp");
  oled_printf_float(50,4，up_pid.kp);//直立P
  
  oled_p6x8str(0,5,"ki");
  oled_printf_float(50,5，up_pid.ki);//直立I
  
  oled_p6x8str(0,6,"kd");
  oled_printf_float(50,6，up_pid.kd);//直立D
  ...
  oled_p6x8str(0,8,"kp");
  oled_printf_float(50,9，speed_pid.kp);//速度P
  
  oled_p6x8str(0,10,"ki");
  oled_printf_float(50,10，speed_pid.ki);//速度I
  
  oled_p6x8str(0,11,"kd");
  oled_printf_float(50,11，speed_pid.kd);//速度D
  
  //转向pid参数
  oled_p6x8str(50,13,"Up Pid");
  
  oled_p6x8str(0,14,"kp");
  oled_printf_float(50,14，direction_pid.kp);//转向P
  
  oled_p6x8str(0,15,"ki");
  oled_printf_float(50,15，direction_pid.ki);//转向I
  
  oled_p6x8str(0,16,"kd");
  oled_printf_float(50,16，direction_pid.kd);//转向D
...
void OledViewCoefficient()//三级页面
{
  ...
  oled_printf_float(50,1，zero_angular_speed_y_data);//转向P
  
  oled_p6x8str(0,2,"change_angle_coefficient");
  oled_printf_float(50,2，change_angle_coefficient);//转向I
  
  oled_p6x8str(0,3,"complementary_coefficient");
  oled_printf_float(50,3，complementary_coefficient);//转向D
